...
namespace FMETP
{
    ...
    public class TextureEncoder : MonoBehaviour
    {
        ...
        private IEnumerator RepeatRequestTextureUpdate()
        {
            yield return new WaitForSeconds(0.4f);
            RequestTextureUpdate();
            StartCoroutine(RepeatRequestTextureUpdate());
        }
        ...
        public CustomClass.TypeData typeData = CustomClass.TypeData.None;
        ...
        private IEnumerator EncodeBytes()
        {
            ...
            if (FastMode)
            {
                ...
                if (AsyncMode && Loom.numThreads < Loom.maxThreads)
                {
                    ...
                    Loom.RunAsync(() =>
                    {
                        ...
                        try { dataByte = (StreamTexture).EncodeToJPG(); } catch { }
                ...
                else
                {
                    ...
                    yield return dataByte = (StreamTexture).EncodeToJPG();
                    if (ignoreSimilarTexture) detectedSimilarTexture = FMCoreTools.CheckSimilarSize(dataByte.Length, lastRawDataByte, similarByteSizeThreshold);
                    lastRawDataByte = dataByte.Length;
            ...
            else
            {
                ...
                dataByte = (StreamTexture).EncodeToJPG();
                if (ignoreSimilarTexture) detectedSimilarTexture = FMCoreTools.CheckSimilarSize(dataByte.Length, lastRawDataByte, similarByteSizeThreshold);
                lastRawDataByte = dataByte.Length;

                if(typeData != CustomClass.TypeData.None)
                {
                    if(typeData == CustomClass.TypeData.ShareScreen)
                    {
                        PhotonManager.instance.SendDataShareScreen(dataByte, StreamTexture.width, StreamTexture.height);
                    }
                    else if(typeData == CustomClass.TypeData.FaceCam)
                    {
                        PhotonManager.instance.SendDataFaceCam(dataByte, StreamTexture.width, StreamTexture.height);
                    }
